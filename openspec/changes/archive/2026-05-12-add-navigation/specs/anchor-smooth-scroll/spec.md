...
单页内包含至少三个可滚动目标 section：首屏（首页）、项目、联系我；各 section 具备稳定 `id`。用户通过导航链接触发平滑滚动至对应 `id` 元素。
...
**Given** 页面存在 `id` 为项目、联系我所约定值的 section  
**When** 用户分别点击「项目」「联系我」  
**Then** 视口以平滑方式滚动至对应 section

### 场景：URL 带 hash 进入

**Given** 用户通过带 hash 的 URL 打开页面（例如 `.../index.html#projects`）  
